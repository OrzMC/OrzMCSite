...
| 文件/目录 | 旧版 Paper | 新版 Paper | Vanilla |
|-----------|-----------|-----------|---------|
| `region/ entities/ poi/` | ✅ 主世界根目录 | ❌ | ❌ |
| `playerdata/ stats/ advancements/` | ✅ 三个平级目录 | ❌ | ❌ |
| `players/` | ❌ | ✅ | ✅ |
| `data/` | ✅ 扁平堆放 | ✅ `data/minecraft/` 命名空间 | ✅ 同 |
| `dimensions/` | ❌ | ✅ | ✅ |
| `session.lock` | ❌ | ✅ | ✅ |
| `uid.dat` | ✅ | ❌ | ❌ |
| `paper-world.yml` | ✅ 根目录一份 | ✅ 每个维度各一份 | ❌ |

#### 玩家数据

| 数据 | 旧版 Paper | 新版 Paper = Vanilla |
|------|-----------|---------------------|
| 玩家 NBT | `playerdata/<UUID>.dat` | `players/data/<UUID>.dat` |
| 统计数据 | `stats/<UUID>.json` | `players/stats/<UUID>.json` |
| 进度 | `advancements/<UUID>.json` | `players/advancements/<UUID>.json` |

#### 维度发现方式

旧版 Paper 通过 `level-name` + 硬编码后缀（`{level-name}_nether` / `{level-name}_the_end`）区分三维度；
新版 Paper = Vanilla 将所有维度统一嵌套在 `dimensions/minecraft/` 下，维度名使用游戏内注册名（`the_nether` / `the_end`），并支持数据包/模组自定义维度。
